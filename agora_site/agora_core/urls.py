...
urlpatterns += patterns('',
    ...
    url(r'^demo/?$', HomeView.as_view(), name='home-demo'),
    ...
    url(r'^(?P<username>[\.\w]+)/(?P<agoraname>[\-\.\w]+)/members/(?P<members_filter>members|admins|active_delegates|membership_requests|denied_membership_requests|admin_membership_requests)/?$',
    ...
    url(r'^(?P<username>[\.\w]+)/(?P<agoraname>[\-\.\w]+)/action/download-user-id/(?P<username2>[\.\w]+)/?$',
        AgoraActionDownloadUserIdView.as_view(),
        name='agora-action-download-user-id'),
    ...
    url(r'^(?P<username>[\.\w]+)/(?P<agoraname>[\-\.\w]+)/election/(?P<electionname>[\-\.\w]+)/action/release_results/?$',
        ReleaseResultsView.as_view(), name='election-action-release-results'),
