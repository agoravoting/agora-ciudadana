...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding field 'Agora.url'
        db.add_column('agora_core_agora', 'url',
                      self.gf('django.db.models.fields.URLField')(default='', max_length=200),
                      keep_default=False)

        # reprocess agoras to add urls
        if not db.dry_run:
            agoras = orm.Agora.objects.all()
            for agora in agoras:
                agora.url = '/' + agora.creator.username + '/' + agora.name
                agora.save()

    def backwards(self, orm):
        # Deleting field 'Agora.url'
        db.delete_column('agora_core_agora', 'url')
    ...
    models = {
        ...
        'agora_core.delegateelectioncount': {
            ...
            'created_at_date': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2013, 7, 3, 0, 0)', 'auto_now_add': 'True', 'blank': 'True'}),
